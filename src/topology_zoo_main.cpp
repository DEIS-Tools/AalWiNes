...
int main(int argc, const char** argv)
{
        ...
        for(auto &r_p : network.routers()){
            ...
            for (const auto& from_interface : r->interfaces()) {
                ...
                for (const auto& to_interface : r_p->interfaces()) {
                    ...
                    auto success = RouteConstruction::make_data_flow(from_interface.get(), to_interface.get(), next_label, cost);
                    if(!success){
    ...
    if(out_topo.is_open()) {
        PRexBuilder::write_prex_topology(network, out_topo);
    } else {
    ...
    if(out_route.is_open()) {
        PRexBuilder::write_prex_routing(network, out_route);
    } else {
